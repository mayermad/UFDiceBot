...
public class MeleeCommand implements ICommand {
    //basic, brawling, cavalry, fencing, flail, parry, poleArm, twoHanded
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "melee", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("melee", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
                channel.sendMessage(ctx.getAuthor().getAsMention() + " du hast diesen Skill nicht gelernt!").queue();
                return;
            }
            int target = skill + SQLManager.getCharacteristic("ws", ctx.getAuthor().getId());
            builder.append(" Dein Melee(").append(speciality).append(") Skill ist ").append(target).append(". \n");
