...
public class RangedCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "ranged", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("ranged", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
                channel.sendMessage(ctx.getAuthor().getAsMention() + " du hast diesen Skill nicht gelernt!").queue();
                return;
            }
            int target = skill + SQLManager.getCharacteristic("bs", ctx.getAuthor().getId());
            builder.append(" Dein Ranged (").append(speciality).append(") Skill ist ").append(target).append(". \n");
