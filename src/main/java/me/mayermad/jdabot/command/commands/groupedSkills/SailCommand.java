...
public class SailCommand implements ICommand {
    //barge, caravel, cog, frigate, wolfship
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "sail", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("sail", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Sail (").append(speciality).append(") Skill ist ").append(target).append(". \n");
