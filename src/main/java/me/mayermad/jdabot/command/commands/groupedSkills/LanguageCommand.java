...
public class LanguageCommand implements ICommand {
    //battleTongue, bretonnian, classical, guilder, khazalid, magick, thief, tilean, albion, eltharin, estalian, gospodarinyi, grumbarth, mootish, norse, reikspiel, queekish, wastelander
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "language", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("language", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Language (").append(speciality).append(") Skill ist ").append(target).append(". \n");
