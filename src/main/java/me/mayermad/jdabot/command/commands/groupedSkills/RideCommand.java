...
public class RideCommand implements ICommand {
    //demigryph, greatWolf, griffon, horse, pegasus
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "ride", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("ride", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Ride (").append(speciality).append(") Skill ist ").append(target).append(". \n");
