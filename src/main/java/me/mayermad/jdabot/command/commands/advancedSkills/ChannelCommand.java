...
public class ChannelCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            int skill = SQLManager.getAdvancedSkill("channel", ctx.getAuthor().getId());
            if (skill == -1 ) {
                channel.sendMessage(ctx.getAuthor().getAsMention() + " du hast diesen Skill nicht gelernt!").queue();
                return;
            }
            int target = skill + SQLManager.getCharacteristic("wp", ctx.getAuthor().getId());
            builder.append(" Dein Channel Skill ist ").append(target).append(". \n");
