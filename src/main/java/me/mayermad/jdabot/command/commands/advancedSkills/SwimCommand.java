...
public class SwimCommand implements ICommand {
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            int skill = SQLManager.getAdvancedSkill("swim", ctx.getAuthor().getId());
            if (skill == -1 ) {
                channel.sendMessage(ctx.getAuthor().getAsMention() + " du hast diesen Skill nicht gelernt!").queue();
                return;
            }
            int target = skill + SQLManager.getCharacteristic("s", ctx.getAuthor().getId());
            builder.append(" Dein Swim Skill ist ").append(target).append(". \n");
