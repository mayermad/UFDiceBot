...
public class PlayCommand implements ICommand {
    //bagpipe, lute, harpsichord, horn, violin
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "play", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("play", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Play (").append(speciality).append(") Skill ist ").append(target).append(". \n");
