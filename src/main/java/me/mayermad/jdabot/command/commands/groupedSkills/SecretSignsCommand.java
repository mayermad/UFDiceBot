...
public class SecretSignsCommand implements ICommand {
    //greyOrder, guild, ranger, scout, thief, vagabond
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "secretSigns", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("secretSigns", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Secret Signs (").append(speciality).append(") Skill ist ").append(target).append(". \n");
