...
public class TrackCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            int skill = SQLManager.getAdvancedSkill("track", ctx.getAuthor().getId());
            if (skill == -1 ) {
                channel.sendMessage(ctx.getAuthor().getAsMention() + " du hast diesen Skill nicht gelernt!").queue();
                return;
            }
            int target = skill + SQLManager.getCharacteristic("i", ctx.getAuthor().getId());
            builder.append(" Dein Track Skill ist ").append(target).append(". \n");
