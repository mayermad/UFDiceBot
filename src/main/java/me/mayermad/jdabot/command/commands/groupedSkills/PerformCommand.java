...
public class PerformCommand implements ICommand {
    //acrobatics, clowning, dancing, firebreathing, juggling, miming, ropeWalking
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "perform", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("perform", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
                channel.sendMessage(ctx.getAuthor().getAsMention() + " du hast diesen Skill nicht gelernt!").queue();
                return;
            }
            int target = skill + SQLManager.getCharacteristic("ag", ctx.getAuthor().getId());
            builder.append(" Dein Perform (").append(speciality).append(") Skill ist ").append(target).append(". \n");
