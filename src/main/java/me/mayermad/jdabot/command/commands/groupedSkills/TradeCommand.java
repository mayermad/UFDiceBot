...
public class TradeCommand implements ICommand {
    //apothecary, calligrapher, chandler, carpenter, cook, embalmer, smith, tanner
    //maybe used with int
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "trade", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("trade", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Trade (").append(speciality).append(") Skill ist ").append(target).append(". \n");
