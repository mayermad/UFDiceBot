...
public class StealthCommand implements ICommand {
    //rural, underground, urban
    @Override
    public void handle(CommandContext ctx) {
        ...
        try {
            ...
            arguments.add(zero);
            String speciality = SQLManager.getStandard( "stealth", ctx.getAuthor().getId());
            ...
            int skill = SQLManager.getGroupedSkill("stealth", speciality, ctx.getAuthor().getId());
            if (skill == -1 ) {
            ...
            builder.append(" Dein Stealth (").append(speciality).append(") Skill ist ").append(target).append(". \n");
